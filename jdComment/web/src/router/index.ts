...
router.beforeEach(async (to, from, next) => {
    ...
    if (!token) {
        // 如果没有 token，且目标路径是登录页，直接跳转到登录页
        if (to.path === '/login' || to.path === '/register') {
            next();
        } else {
            // 没有 token，跳转到登录页或注册页
            const redirectPath = to.path === '/login' ? '/' : to.path;
            next(`/login?redirect=${redirectPath}&params=${JSON.stringify(to.query ? to.query : to.params)}`);
        }
        NProgress.done();
    } else {
        // 如果有 token，处理其他逻辑
        if (to.path === '/login') {
            // 已登录状态下，如果目标是登录页，跳转到首页
            next('/home');
